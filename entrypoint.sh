...
/usr/local/bin/invoke waitfordbs >> /usr/src/nexus/invoke.log
echo "waitfordbs task done"

echo "running migrations"
/usr/local/bin/invoke migrations >> /usr/src/nexus/invoke.log
...
then
    ...
    then
        ...
        /usr/local/bin/invoke prepare >> /usr/src/nexus/invoke.log
        ...
        if [ ${IS_FIRST_START} = "true" ] || [ ${IS_FIRST_START} = "True" ] || [ ${FORCE_REINIT} = "true" ]  || [ ${FORCE_REINIT} = "True" ] || [ ! -e "/mnt/volumes/statics/geonode_init.lock" ]; then
            /usr/local/bin/invoke updategeoip >> /usr/src/nexus/invoke.log
            echo "updategeoip task done"
            /usr/local/bin/invoke fixtures >> /usr/src/nexus/invoke.log
            echo "fixture task done"
            /usr/local/bin/invoke monitoringfixture >> /usr/src/nexus/invoke.log
            echo "monitoringfixture task done"
            /usr/local/bin/invoke initialized >> /usr/src/nexus/invoke.log
            echo "initialized"
        fi

        echo "refresh static data"
        /usr/local/bin/invoke statics >> /usr/src/nexus/invoke.log
        echo "static data refreshed"
        /usr/local/bin/invoke waitforgeoserver >> /usr/src/nexus/invoke.log
        echo "waitforgeoserver task done"
        /usr/local/bin/invoke geoserverfixture >> /usr/src/nexus/invoke.log
        echo "geoserverfixture task done"
        /usr/local/bin/invoke updateadmin >> /usr/src/nexus/invoke.log
